...
<div
  class="col-span-3 row-span-2 glassmorphism rounded-2xl overflow-hidden min-h-[400px] tech-glow"
>
      ...
      <div class="flex-1 space-y-3">
        ...
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-sm text-gray-300 font-mono capitalize">
          ...
          <div class="grid grid-cols-4 gap-1">
            {#each weatherData.today as forecast}
              <div class="text-center p-1.5 bg-gray-900/50 rounded-lg border border-gray-700/50">
                ...
                <svelte:component
                  this={getWeatherIcon(forecast.icon)}
                  class="h-5 w-5 mx-auto text-yellow-400 my-1"
                />
              ...
              <div class="flex items-center space-x-2">
                ...
                <div>
                  ...
                  <p class="text-xs font-mono text-gray-400">
                    Précip. {weatherData.tomorrow.precipitation}%
                  </p>
                </div>
              </div>
            ...
            <div class="grid grid-cols-5 gap-1 max-h-40 overflow-y-auto">
              {#each weatherData.tomorrow.hourly as forecast}
                ...
                >
                  ...
                  <svelte:component
                    this={getWeatherIcon(forecast.icon)}
                    class="h-4 w-4 mx-auto text-cyan-400 my-1"
                  />
                  <div class="text-xs font-mono text-white">{forecast.temp}°</div>
                  {#if forecast.precipitation > 0}
                    <div class="text-xs font-mono text-blue-400">{forecast.precipitation}%</div>
                  {/if}
                </div>
              {/each}
            </div>
