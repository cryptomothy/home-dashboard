...
export const GET: RequestHandler = async ({ url }) => {
  try {
    ...
    console.log('🚗 CORS Proxy Communauto: Récupération des véhicules pour CityId:', cityId);
    if (geoParams.length > 0) {
      console.log('🚗 Paramètres géolocalisation:', geoParams.join(', '));
    }

    console.log("🚗 URL de l'API Communauto CORS Proxy:", apiUrl);

    // Utiliser un autre service de proxy CORS
    const proxyUrl = `https://cors-anywhere.herokuapp.com/${apiUrl}`;

    console.log('🚗 URL du CORS proxy:', proxyUrl);
    ...
    const response = await fetch(proxyUrl, {
      ...
      headers: {
        ...
        Origin: 'https://walkiworki.vercel.app',
        'X-Requested-With': 'XMLHttpRequest',
      },
      signal: AbortSignal.timeout(20000), // 20 secondes de timeout
    });

    console.log('🚗 Réponse du CORS proxy:', response.status, response.statusText);

    if (!response.ok) {
      const errorText = await response
        .text()
        .catch(() => "Impossible de lire le contenu de l'erreur");
      console.error('🚗 Erreur CORS proxy:', {
        status: response.status,
        statusText: response.statusText,
        body: errorText,
      });
      throw new Error(
        `Erreur CORS proxy: ${response.status} ${response.statusText} - ${errorText}`,
      );
    }

    const data = await response.json();

    console.log(`🚗 CORS Proxy Communauto: ${data.totalNbVehicles} véhicules récupérés`);
    ...
    console.error('🚗 Erreur CORS proxy Communauto:', error);

    return json(
      {
        error: 'Erreur lors de la récupération des véhicules Communauto (CORS proxy)',
