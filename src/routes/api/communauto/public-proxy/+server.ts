...
export const GET: RequestHandler = async ({ url }) => {
  try {
    ...
    console.log('🚗 Public Proxy Communauto: Récupération des véhicules pour CityId:', cityId);
    if (geoParams.length > 0) {
      console.log('🚗 Paramètres géolocalisation:', geoParams.join(', '));
    }

    console.log("🚗 URL de l'API Communauto Public Proxy:", apiUrl);

    // Utiliser un service de proxy public
    const proxyUrl = `https://thingproxy.freeboard.io/fetch/${apiUrl}`;

    console.log('🚗 URL du proxy public:', proxyUrl);
    ...
    console.log('🚗 Réponse du proxy public:', response.status, response.statusText);

    if (!response.ok) {
      const errorText = await response
        .text()
        .catch(() => "Impossible de lire le contenu de l'erreur");
      console.error('🚗 Erreur proxy public:', {
        status: response.status,
        statusText: response.statusText,
        body: errorText,
      });
      throw new Error(
        `Erreur proxy public: ${response.status} ${response.statusText} - ${errorText}`,
      );
    }

    const data = await response.json();

    console.log(`🚗 Public Proxy Communauto: ${data.totalNbVehicles} véhicules récupérés`);
    ...
    console.error('🚗 Erreur proxy public Communauto:', error);

    return json(
      {
        error: 'Erreur lors de la récupération des véhicules Communauto (proxy public)',
